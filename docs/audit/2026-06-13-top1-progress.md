...
- Мастер-ТЗ создано.
- Последний подтвержденный rank-baseline остается из PR-CY: `94` запроса, средняя позиция `81.55`, видимость `0.90`.
- Production sitemap на 21.06.2026: `112` URL, все проверенные ключевые URL живые.
- Свежий browser-check GSC/Яндекс на 21.06.2026 снова не получен: без активной авторизации прямые маршруты уходят на public/login, поэтому live-статусы индексации и переобхода нужно добрать отдельным browser-проходом после входа.
...
| 6. Контент-кластеры | Выполнен + усилен | 29.06.2026 | Контентные статьи связаны с релевантными статическими кейсами, чтобы блог передавал вес на портфолио |
| 7. Портфолио и кейсы | Выполнен | 29.06.2026 | Кейсы усилены proof-блоком: задача, решение, материалы, срок/бюджет, статус изображения и переходы к money/trust-страницам |
| 8. Geo-страницы | Выполнен | 29.06.2026 | Городские страницы получили блок локальных условий без фальшивой привязки чужих фото к городам |
| 9. Внутренняя перелинковка | Выполнен | 29.06.2026 | Блог ведет на кейсы; кейсы ведут на цены, калькулятор, материалы, город, гарантию и отзывы; geo ведет на цены, калькулятор, портфолио и гарантию |
| 10. Внешние сигналы | Подготовлен | 29.06.2026 | Создан NAP-пакет, список 50 площадок/направлений, UTM-шаблоны, план отзывов и 14-дневный чеклист; аккаунтные размещения остаются ручным шагом |
| 11. GSC/Яндекс/PR-CY переобход | Не начат | — | Выполнять после крупных деплоев |
| 12. Еженедельный SEO-контроль | Частично выполнен | 21.06.2026 | Production weekly-check обновлен: sitemap уже 112 URL; свежие GSC/Яндекс/PR-CY данные не получены без авторизации |
...
```text
Дата: 29.06.2026
Этап: 6-10. Контентные связи, портфолио, geo, внутренняя перелинковка, внешние сигналы
Что изменено: блоговые статьи получили связи с релевантными кейсами; страницы кейсов получили proof-блок и переходы к ценам, калькулятору, материалам, geo, гарантии и отзывам; городские страницы получили блок локальных условий; создан docs/audit/2026-06-29-stage-7-10-trust-linking-external-sprint.md с NAP-пакетом, 50 площадками, UTM и планом отзывов.
Проверки: typecheck, seo:check, sitemap:check, build, smoke/mobile/browser-check после правок.
Деплой: выполнить после успешных проверок командой Timeweb update-production.sh work.
Production URL: https://kuhni.minsk.by/blog/skolko-stoit-kuhnya-na-zakaz-minsk-2026, https://kuhni.minsk.by/portfolio/seraya-uglovaya-kuhnya-novostrojka-minsk, https://kuhni.minsk.by/locations/borisov
GSC/Яндекс: после деплоя можно отправить обновленные URL на переобход только через авторизованный браузерный сеанс; индексация не гарантируется.
Осталось: разместить/обновить внешние профили с единым NAP, собрать первые отзывы на Яндекс/Google/2ГИС/сайт, повторить GSC/Яндекс browser-check после входа.
```
...
Дата: 21.06.2026
Этап: 12. Еженедельный SEO-контроль
Что изменено: создан docs/audit/2026-06-21-stage-15-weekly-seo-control.md; подтверждены production robots/sitemap/canonical/H1 и факт роста static sitemap до 112 URL.
Проверки: production /robots.txt 200; /sitemap.xml 200, X-Sitemap-Source: static, 112 URL; ключевые URL 200, self-canonical, indexable; redirect hygiene по http/www/legacy slugs сохранена.
Деплой: не требовался.
Production URL: https://kuhni.minsk.by/, https://kuhni.minsk.by/catalog, https://kuhni.minsk.by/locations/minskaya-oblast, https://kuhni.minsk.by/prices, https://kuhni.minsk.by/sitemap.xml
GSC/Яндекс: свежий browser-login в этой сессии не состоялся; неавторизованные маршруты ведут на public/login, поэтому live-статусы индексации и переобхода не обновлены.
Осталось: открыть GSC и Яндекс Вебмастер в авторизованном browser-сеансе и снять свежий статус sitemap/coverage/recrawl; отдельно получить новый PR-CY или PixelPlus экспорт позиций.
```
