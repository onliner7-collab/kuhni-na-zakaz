...
- Яндекс Вебмастер:
  - sitemap `https://kuhni.minsk.by/sitemap.xml` уже добавлен вручную и найден в `robots.txt`, статус `ок`; на момент проверки интерфейс Яндекса показывал последнюю загрузку `29.05.2026 15:28` и 79 ссылок.
  - В инструменте `Переобход страниц` отправлены все 12 URL волн 2 и 3.
  - Статус отправленных URL: `В очереди`, время отправки `01.06.2026 10:43`; после отправки осталось 138 адресов дневного лимита.
