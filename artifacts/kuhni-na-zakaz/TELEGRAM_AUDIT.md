...
> **Статус (2026-05-13, после этапа 9):** этапы 1–9 завершены. Fallback на
> `process.env.TELEGRAM_CHAT_IDS` **удалён** из `lib/telegram.ts`, получатели
> теперь только из БД (`TelegramRecipient.active = true`). Пункты ниже про
> «fallback пока сохраняем» — историческое описание состояния до Этапа 9 и
> сохранены как контекст принятия решений.
