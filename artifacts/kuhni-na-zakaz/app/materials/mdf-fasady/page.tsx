...
const limitations = [
  ...
  "Срок службы и технические характеристики уточняются для выбранного покрытия.",
...
export default function MdfFacadesPage() {
  ...
  return <ExploreContextProvider sourceRoute="/materials/mdf-fasady">
    ...
    <main className="overflow-x-clip bg-[#f7f5f2] pb-28 text-stone-950 md:pb-16">
      <section className="border-b border-stone-200 bg-white">
        <div className="container-site py-12 md:py-20">
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_.8fr] lg:items-center"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-violet-800">Поверхности фасадов МДФ</p><h1 className="mt-3 max-w-4xl text-4xl font-extrabold leading-tight tracking-tight md:text-6xl">Как сравнить поверхность фасадов МДФ</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-stone-600">Поверхность → крупный план → сравнение → ограничения → сочетание со стилем или планировкой → расчёт.</p><a href="#surface" className="mt-7 inline-flex min-h-12 items-center gap-2 rounded-full bg-violet-900 px-5 font-bold text-white focus-visible:outline focus-visible:ring-2 focus-visible:ring-violet-700 focus-visible:ring-offset-2">Начать с поверхности <ArrowRight className="h-4 w-4" aria-hidden /></a></div><figure className="overflow-hidden rounded-[2rem] border border-stone-200 bg-white"><img src="/media/pilots/mdf-fasady/webp/mdf-surface-closeup.webp" alt="Крупный план матовой поверхности фасада МДФ" width="1200" height="800" fetchPriority="high" className="aspect-[3/2] h-auto w-full object-cover" /><figcaption className="px-5 py-3 text-sm text-stone-600">AI‑концепт фактуры; точный оттенок и поверхность проверяются по образцу.</figcaption></figure></div>
        </div>
      </section>
      ...
      <div className="container-site py-14 md:py-20">
        ...
        <section id="limits" className="mt-16 scroll-mt-24 overflow-hidden rounded-[2rem] border border-amber-200 bg-amber-50" aria-labelledby="limits-title"><div className="grid lg:grid-cols-[.75fr_1.25fr]"><figure className="bg-white"><img src="/media/pilots/mdf-fasady/webp/mdf-surface-cutaway.webp" alt="Условная схема основы, покрытия и торца фасада МДФ" width="1200" height="675" loading="lazy" decoding="async" className="aspect-[16/9] h-full w-full object-cover" /><figcaption className="px-5 py-3 text-sm text-stone-600">Техническая AI‑иллюстрация, не паспорт изделия.</figcaption></figure><div className="p-6 md:p-9"><div className="flex items-center gap-3"><CircleAlert className="h-7 w-7 text-amber-800" aria-hidden /><h2 id="limits-title" className="text-3xl font-bold">Что нужно уточнить</h2></div><ul className="mt-6 grid gap-3">{limitations.map((item) => <li key={item} className="flex gap-3 rounded-2xl bg-white p-4 text-sm leading-6"><span aria-hidden="true" className="mt-2 h-2 w-2 shrink-0 rounded-full bg-amber-700" />{item}</li>)}</ul></div></div></section>
