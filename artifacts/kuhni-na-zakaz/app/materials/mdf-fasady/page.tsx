  ...
  return <ExploreContextProvider sourceRoute="/materials/mdf-fasady">
    ...
    <main className="overflow-x-clip bg-[#f7f5f2] pb-28 text-stone-950 md:pb-16">
      ...
      <div className="container-site py-14 md:py-20">
        ...
        <section id="limits" className="mt-16 scroll-mt-24 overflow-hidden rounded-[2rem] border border-amber-200 bg-amber-50" aria-labelledby="limits-title"><div className="grid lg:grid-cols-[.75fr_1.25fr]"><figure className="min-h-0 bg-white"><img src="/media/pilots/mdf-fasady/webp/mdf-surface-cutaway.webp" alt="Условная схема основы, покрытия и торца фасада МДФ" width="1200" height="675" loading="lazy" decoding="async" className="aspect-[16/9] h-auto w-full object-cover lg:h-full" /></figure><div className="p-6 md:p-9"><div className="flex items-start gap-3"><CircleAlert className="mt-1 h-7 w-7 shrink-0 text-amber-800" aria-hidden /><h2 id="limits-title" className="min-w-0 text-3xl font-bold leading-tight">Что нужно уточнить</h2></div><ul className="mt-6 grid gap-3">{limitations.map((item) => <li key={item} className="flex gap-3 rounded-2xl bg-white p-4 text-sm leading-6"><span aria-hidden="true" className="mt-2 h-2 w-2 shrink-0 rounded-full bg-amber-700" />{item}</li>)}</ul></div></div></section>
