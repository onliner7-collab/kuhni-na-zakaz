...
const schema = z.object({
  ...
  material: z.string().default(""), thicknessMm: z.number().int().default(38),
  colorName: z.string().default(""), imageUrl: z.string().default(""),
  pricePerMeter: z.number().int().min(0).default(0),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.kitchenCountertop.create({ data: parsed.data }).catch(() => null);
