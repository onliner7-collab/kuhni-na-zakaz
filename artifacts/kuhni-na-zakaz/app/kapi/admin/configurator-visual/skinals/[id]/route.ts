...
const schema = z.object({
  ...
  material: z.string().optional(), colorName: z.string().optional(),
  imageUrl: z.string().optional(), pricePerSqMeter: z.number().int().min(0).optional(),
...
export async function PUT(req: NextRequest, { params }: P) {
  ...
  const item = await prisma.kitchenSkinal.update({ where: { id: parseInt(id) }, data: parsed.data }).catch(() => null);
...
export async function DELETE(_req: NextRequest, { params }: P) {
  ...
  const { id } = await params;
  await prisma.kitchenSkinal.delete({ where: { id: parseInt(id) } }).catch(() => null);
