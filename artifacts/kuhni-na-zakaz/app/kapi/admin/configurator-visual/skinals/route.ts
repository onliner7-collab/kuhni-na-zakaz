...
const schema = z.object({
  ...
  material: z.string().default(""), colorName: z.string().default(""),
  imageUrl: z.string().default(""), pricePerSqMeter: z.number().int().min(0).default(0),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.kitchenSkinal.create({ data: parsed.data }).catch(() => null);
