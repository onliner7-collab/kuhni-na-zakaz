...
const schema = z.object({
  ...
  handleType: z.enum(["STANDARD","RECESSED","PUSH_TO_OPEN","INTEGRATED"]),
  material: z.string().default(""), finishName: z.string().default(""),
  imageUrl: z.string().default(""), pricePerPiece: z.number().int().min(0).default(0),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.kitchenHandle.create({ data: parsed.data }).catch(() => null);
