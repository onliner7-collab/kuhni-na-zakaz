...
const schema = z.object({
  ...
  handleType: z.enum(["STANDARD","RECESSED","PUSH_TO_OPEN","INTEGRATED"]).optional(),
  material: z.string().optional(), finishName: z.string().optional(),
...
export async function PUT(req: NextRequest, { params }: P) {
  ...
  const item = await prisma.kitchenHandle.update({ where: { id: parseInt(id) }, data: parsed.data }).catch(() => null);
...
export async function DELETE(_req: NextRequest, { params }: P) {
  ...
  const { id } = await params;
  await prisma.kitchenHandle.delete({ where: { id: parseInt(id) } }).catch(() => null);
