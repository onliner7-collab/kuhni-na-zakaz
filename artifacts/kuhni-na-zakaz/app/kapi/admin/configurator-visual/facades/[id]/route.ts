...
const schema = z.object({
  ...
  material: z.string().optional(), finish: z.string().optional(),
  colorHex: z.string().optional(), colorName: z.string().optional(),
  imageUrl: z.string().optional(), priceMultiplier: z.number().min(0).optional(),
...
export async function PUT(req: NextRequest, { params }: P) {
  ...
  const item = await prisma.kitchenFacade.update({ where: { id: parseInt(id) }, data: parsed.data }).catch(() => null);
...
export async function DELETE(_req: NextRequest, { params }: P) {
  ...
  const { id } = await params;
  await prisma.kitchenFacade.delete({ where: { id: parseInt(id) } }).catch(() => null);
