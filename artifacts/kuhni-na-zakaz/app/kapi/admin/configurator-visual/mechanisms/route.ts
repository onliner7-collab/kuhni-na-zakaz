...
const schema = z.object({
  ...
  mechanismType: z.enum(["HINGE","LIFT","DRAWER","TANDEM","CORNER"]),
  loadKg: z.number().int().min(0).default(0),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.kitchenMechanism.create({ data: parsed.data }).catch(() => null);
