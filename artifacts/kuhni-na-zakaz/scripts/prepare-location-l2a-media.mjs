...
const evidenceRoot = path.resolve(projectRoot, "..", "..", "artifacts", "location-visual-corrective", "l2a", "contact-sheets");
const cities = ["smolevichi", "dzerzhinsk", "zaslavl", "logoisk"];

await fs.mkdir(evidenceRoot, { recursive: true });

for (const city of cities) {
  const directory = path.join(publicRoot, `${city}-visual-l2a`);
...
console.log("Prepared 16 L2A masters with WebP/AVIF parity, mobile WebP derivatives and four contact sheets.");
