...
const evidenceRoot = path.resolve(projectRoot, "..", "..", "artifacts", "location-visual-corrective", "l3a", "contact-sheets");
const cities = ["berezino", "stolbtsy", "uzda"];

await fs.mkdir(evidenceRoot, { recursive: true });

for (const city of cities) {
  const directory = path.join(publicRoot, `${city}-visual-l3a`);
...
console.log("Prepared 12 L3A masters with WebP/AVIF parity, mobile WebP derivatives and three contact sheets.");
