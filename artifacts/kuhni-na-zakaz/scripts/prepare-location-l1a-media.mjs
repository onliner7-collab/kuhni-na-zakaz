...
const evidenceRoot = path.resolve(projectRoot, "..", "..", "artifacts", "location-visual-corrective", "l1a", "contact-sheets");
const cities = ["vitebsk", "grodno", "brest", "mogilev"];

await fs.mkdir(evidenceRoot, { recursive: true });

for (const city of cities) {
  const directory = path.join(publicRoot, `${city}-visual-l1a`);
...
console.log("Prepared 16 L1A masters with WebP/AVIF parity, mobile WebP derivatives and four contact sheets.");
