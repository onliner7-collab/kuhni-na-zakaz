...
const l1bRoutes = ["/locations/molodechno", "/locations/zhodino", "/locations/slutsk", "/locations/maryina-gorka"] as const;
...
test.describe("location visual corrective L1B", () => {
  for (const route of l1bRoutes) {
  ...
  for (const width of [360, 412, 768, 1440]) {
    test(`family-center representative remains responsive at ${width}px`, async ({ page }) => {
      await page.setViewportSize({ width, height: width >= 768 ? 960 : width === 412 ? 915 : 800 });
      await assertSeoAndLayout(page, "/locations/molodechno");
  ...
  test("keyboard and reduced motion keep the four states usable", async ({ page }) => {
    ...
    await page.goto("/locations/zhodino", { waitUntil: "networkidle" });
