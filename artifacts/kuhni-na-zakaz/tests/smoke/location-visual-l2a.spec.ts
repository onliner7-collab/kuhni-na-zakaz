...
const l2aRoutes = ["/locations/smolevichi", "/locations/dzerzhinsk", "/locations/zaslavl", "/locations/logoisk"] as const;
...
test.describe("location visual corrective L2A", () => {
  for (const route of l2aRoutes) {
    ...
    test(`${route}: mobile visual-first layout at 390px`, async ({ page }) => {
      ...
      expect(await page.locator("h1").evaluate((h1) => h1.compareDocumentPosition(document.querySelector("[data-location-visual-explorer]")!) & Node.DOCUMENT_POSITION_FOLLOWING)).toBeTruthy();
  ...
  for (const width of [360, 412, 768, 1440]) {
    test(`minsk-satellite representative remains responsive at ${width}px`, async ({ page }) => {
      await page.setViewportSize({ width, height: width >= 768 ? 960 : width === 412 ? 915 : 800 });
      await assertSeoAndLayout(page, "/locations/smolevichi");
  ...
  test("keyboard and reduced motion keep the four states usable", async ({ page }) => {
    ...
    await page.goto("/locations/logoisk", { waitUntil: "networkidle" });
