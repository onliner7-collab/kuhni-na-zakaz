...
const l2bRoutes = ["/locations/vileyka", "/locations/nesvizh", "/locations/volozhin", "/locations/myadel"] as const;
...
test.describe("location visual corrective L2B", () => {
  for (const route of l2bRoutes) {
  ...
  for (const width of [360, 412, 768, 1440]) {
    test(`minsk-satellite representative remains responsive at ${width}px`, async ({ page }) => {
      ...
      await assertSeoAndLayout(page, "/locations/vileyka");
  ...
  test("keyboard and reduced motion keep the four states usable", async ({ page }) => {
    ...
    await page.goto("/locations/volozhin", { waitUntil: "networkidle" });
