...
const l1aRoutes = ["/locations/vitebsk", "/locations/grodno", "/locations/brest", "/locations/mogilev"] as const;
...
test.describe("location visual corrective L1A", () => {
  for (const route of l1aRoutes) {
  ...
  for (const width of [360, 412, 768, 1440]) {
    test(`regional-center representative remains responsive at ${width}px`, async ({ page }) => {
      await page.setViewportSize({ width, height: width >= 768 ? 960 : width === 412 ? 915 : 800 });
      await assertSeoAndLayout(page, "/locations/vitebsk");
  ...
  test("keyboard and reduced motion keep the four states usable", async ({ page }) => {
    ...
    await page.goto("/locations/grodno", { waitUntil: "networkidle" });
