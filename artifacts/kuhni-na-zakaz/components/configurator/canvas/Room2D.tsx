...
export function Room2D({
  ...
  const stageHeightClass = readOnly
  ...
  function wallStartXY(wallSide: string): [number, number] {
    switch (wallSide) {
      ...
      case "left":
        return [ox, oy];
      default:
        return [ox + W / 2, oy + D / 2]; // island
    }
  }

  function getModuleRect(pm: PlacedModule, catalogItem: CatalogModule) {
    const ws = pm.wallSide;
    const depthPx = ["LOWER", "SINK", "CORNER", "DRAWER"].includes(
      catalogItem.moduleType,
    )
  ...
  return (
    <div className="w-full rounded-2xl border bg-stone-50 p-3 sm:p-4">
      <div className={`mx-auto w-full max-w-[560px] ${stageHeightClass}`}>
        <svg
          ref={svgRef}
          width="100%"
          height="100%"
        ...
        >
          ...
          {protrusions.map((p) => {
            ...
            const px =
              p.wallIndex === 3
                ? ox
                : p.wallIndex === 1
                  ? ox + W - p.depthCm * scale
                  : ox + p.offsetCm * scale;
            const py =
              p.wallIndex === 0
                ? oy
                : p.wallIndex === 2
                  ? oy + D - p.depthCm * scale
                  : oy + p.offsetCm * scale;
          ...
          {openings.map((o) => {
            ...
            if (o.wallIndex === 0) {
              rx = ox + o.offsetCm * scale;
              ry = oy - WALL_THICKNESS;
              rw = wPx;
              rh = tPx;
            } else if (o.wallIndex === 2) {
              rx = ox + o.offsetCm * scale;
              ry = oy + D;
              rw = wPx;
              rh = tPx;
            } else if (o.wallIndex === 3) {
              rx = ox - WALL_THICKNESS;
              ry = oy + o.offsetCm * scale;
              rw = tPx;
              rh = wPx;
            } else {
              rx = ox + W;
            ...
            return (
              <g key={o.id}>
                <rect
                  ...
                  width={rw}
                  height={rh}
                  fill={o.type === "door" ? "#fde68a" : "#bae6fd"}
                  stroke="none"
                />
                {/* Door arc */}
                {o.type === "door" && isH && (
          ...
          {placedModules.map((pm) => {
            ...
            const isLower = ["LOWER", "SINK", "CORNER", "DRAWER"].includes(
              cat.moduleType,
            );
