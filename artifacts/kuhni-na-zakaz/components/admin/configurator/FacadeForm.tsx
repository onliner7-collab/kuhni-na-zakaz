...
export default function FacadeForm({ initial, id }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    priceMultiplier: initial?.priceMultiplier ?? 1.0,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    const url = id
      ? `/kapi/admin/configurator-visual/facades/${id}`
      : "/kapi/admin/configurator-visual/facades";
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...form,
        priceMultiplier: parseFloat(String(form.priceMultiplier)),
    ...
    router.push("/admin/configurator-visual/facades");
    router.refresh();
  }
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            step="0.01"
            className={inp}
            value={Number(form.priceMultiplier)}
            onChange={(e) =>
              setForm((f) => ({ ...f, priceMultiplier: e.target.value }))
            }
          />
        </div>
        <div>
