...
const HANDLE_TYPES = ["STANDARD", "RECESSED", "PUSH_TO_OPEN", "INTEGRATED"];
...
export default function HandleForm({ initial, id }: Props) {
  const router = useRouter();
  const [form, setForm] = useState({
    name: initial?.name ?? "",
    slug: initial?.slug ?? "",
    handleType: initial?.handleType ?? "STANDARD",
    material: initial?.material ?? "",
    finishName: initial?.finishName ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    const url = id
      ? `/kapi/admin/configurator-visual/handles/${id}`
      : "/kapi/admin/configurator-visual/handles";
    ...
    router.push("/admin/configurator-visual/handles");
    router.refresh();
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-xl">
      ...
      <div>
        <label className="block text-sm font-medium mb-1">Тип ручки *</label>
        <select
          className={inp}
          value={String(form.handleType)}
          onChange={(e) =>
            setForm((f) => ({ ...f, handleType: e.target.value }))
          }
        >
          {HANDLE_TYPES.map((t) => (
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Отделка</label>
          <input
            className={inp}
            value={String(form.finishName)}
            onChange={(e) =>
              setForm((f) => ({ ...f, finishName: e.target.value }))
            }
          />
        </div>
      </div>
