...
export default function CountertopForm({ initial, id }: Props) {
  const router = useRouter();
  const [form, setForm] = useState({
    name: initial?.name ?? "",
    slug: initial?.slug ?? "",
    material: initial?.material ?? "",
    thicknessMm: initial?.thicknessMm ?? 38,
    colorName: initial?.colorName ?? "",
    imageUrl: initial?.imageUrl ?? "",
    pricePerMeter: initial?.pricePerMeter ?? 0,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    const url = id
      ? `/kapi/admin/configurator-visual/countertops/${id}`
      : "/kapi/admin/configurator-visual/countertops";
    ...
    router.push("/admin/configurator-visual/countertops");
    router.refresh();
  }
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="number"
            className={inp}
            value={Number(form.thicknessMm)}
            onChange={(e) =>
              setForm((f) => ({ ...f, thicknessMm: e.target.value }))
            }
          />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="number"
            className={inp}
            value={Number(form.pricePerMeter)}
            onChange={(e) =>
              setForm((f) => ({ ...f, pricePerMeter: e.target.value }))
            }
          />
        </div>
      </div>
