...
const configs: Record<string, Config> = {
  ...
  "p-obraznye-kuhni": {
    ...
    options: [
      { id: "overview", label: "Три стороны", result: "Сначала оцените весь внутренний контур.", caution: "остаётся ли свободный вход в рабочую зону" },
      { id: "open", label: "Открыта техника", result: "Открытые дверцы занимают часть прохода.", caution: "можно ли безопасно пройти мимо техники" },
  ...
  "malenkie-kuhni": {
    ...
    visualFrames: smallKitchenFrames(),
...
function uFrames(): LayoutVisualFrame[] {
  const base = "/media/visual-rescue/p-obraznye-kuhni";
  return [
    visualFrame(base, "u-overview", "Три стороны", "П-образная кухня с тремя рабочими сторонами и центральным проходом", "Три стороны дают много рабочей поверхности вокруг прохода.", "остаётся ли свободный вход в рабочую зону"),
    visualFrame(base, "u-clear-passage", "Внутренний проход", "П-образная кухня с полностью свободным внутренним проходом", "Свободный проход нужно оценивать до открывания техники.", "ширину между противоположными фасадами по замеру"),
    visualFrame(base, "u-open-appliances", "Открыта техника", "П-образная кухня с открытыми духовкой и посудомоечной машиной", "Открытые дверцы занимают часть внутреннего прохода.", "можно ли безопасно пройти мимо открытой техники"),
    visualFrame(base, "u-solo", "Готовит один", "П-образная кухня с одним открытым ящиком у варочной зоны", "Один активный маршрут можно собрать между мойкой и варочной.", "не пересекает ли открытый ящик основной путь"),
    visualFrame(base, "u-pair", "Готовят вдвоём", "П-образная кухня с открытыми ящиками по обе стороны и посудомоечной машиной", "Два одновременных действия заметно сокращают свободное пространство.", "можно ли разойтись при открытых ящиках"),
    visualFrame(base, "u-l-compare", "Сравнить с L-формой", "Та же кухня без правой стороны как угловая альтернатива", "L-форма освобождает одну сторону, но уменьшает рабочий контур.", "что важнее: свободный проход или третья рабочая сторона"),
  ];
}

function islandFrames(): LayoutVisualFrame[] {
  const base = "/media/visual-rescue/kuhni-s-ostrovom";
  return [
    visualFrame(base, "island-overview", "Общий вид", "Кухня-гостиная с закрытым центральным островом и свободными проходами", "Сначала оцените остров целиком и его связь с основной линией.", "фактические проходы по обмерам помещения"),
    visualFrame(base, "island-passage", "Проход", "Кухня с островом с хорошо видимыми свободными проходами по сторонам", "Свободное пространство вокруг острова важнее условной площади комнаты.", "открывание ящиков, техники и маршрут в гостиную"),
    visualFrame(base, "island-prep", "Подготовка", "Остров с рабочей доской и открытым ящиком для столовых приборов", "Рабочая роль оставляет остров свободным для подготовки продуктов.", "розетки и безопасный проход у открытого ящика"),
    visualFrame(base, "island-storage", "Хранение", "Остров с двумя открытыми широкими ящиками для посуды и кастрюль", "Широкие ящики разгружают основную линию кухни.", "сторону открывания и глубину модулей"),
    visualFrame(base, "island-seating", "Посадка", "Остров с тремя деревянными стульями и местом для сервировки", "Посадка превращает остров в место общения, но занимает проход со стороны гостиной.", "свес столешницы, место для ног и проход за стульями"),
    visualFrame(base, "island-roles-compare", "Сравнить роли", "Второй ракурс той же кухни с закрытым островом без предметов", "Чистый второй ракурс помогает сравнить рабочую, складскую и социальную роли.", "какая одна роль острова должна быть главной"),
  ];
}

function smallKitchenFrames(): LayoutVisualFrame[] {
  const base = "/media/visual-rescue/malenkie-kuhni";
  return [
    visualFrame(base, "small-overview", "Исходное пространство", "Компактная Г-образная кухня в узком помещении с окном", "В исходном состоянии видны все обязательные зоны и свободный пол.", "реальные габариты помещения и открывания двери"),
    visualFrame(base, "small-worktop", "Столешница", "Компактная кухня со свободной столешницей и одной разделочной доской", "Непрерывный участок столешницы становится главным рабочим ресурсом.", "куда убирать сушку и мелкую технику после использования"),
    visualFrame(base, "small-storage", "Хранение", "Маленькая кухня с открытыми ящиками и верхним шкафом", "Вертикальные и узкие зоны добавляют хранение без расширения кухни.", "доступ к верхним полкам и свободный проход при открытых ящиках"),
    visualFrame(base, "small-appliances", "Техника", "Компактная кухня с открытыми духовкой и холодильником", "Открытая техника показывает, сколько прохода остаётся в ежедневном сценарии.", "конфликт дверец и остаток рабочей поверхности"),
    visualFrame(base, "small-passage", "Проход", "Вид из дверного проёма на свободный проход компактной кухни", "Свободный маршрут до окна и мойки не должен зависеть от закрытых фасадов.", "ширину прохода при одновременном использовании кухни"),
    visualFrame(base, "small-compromise", "Компромисс", "Компактная кухня с дополнительным верхним хранением и техникой на столешнице", "Больше хранения и техники визуально сокращают свободную рабочую зону.", "что важнее сохранить именно в вашем сценарии"),
  ];
}

function ceilingFrames(): LayoutVisualFrame[] {
  const base = "/media/visual-rescue/kuhni-do-potolka";
  return [
    visualFrame(base, "ceiling-overview", "Общий фасад", "Кухня с единым фасадом хранения от пола до потолка", "Общий фасад показывает распределение высоты без обещания применимости.", "высоту потолка в нескольких точках и геометрию стен"),
    visualFrame(base, "ceiling-daily-zone", "Ежедневная зона", "Кухня до потолка с открытым ящиком и шкафом ежедневного доступа", "Часто используемые вещи остаются на уровне рук и глаз.", "удобство открывания рядом с рабочей зоной"),
    visualFrame(base, "ceiling-upper-tier", "Верхний ярус", "Кухня до потолка с открытыми пустыми секциями верхнего яруса", "Верхний ярус добавляет объём, но не ежедневную доступность.", "вес вещей и безопасный способ доступа"),
    visualFrame(base, "ceiling-seasonal", "Сезонное хранение", "Верхние шкафы кухни с корзинами и редко используемой посудой", "Редкие и лёгкие вещи логично отделить от ежедневной зоны.", "не перегружать верх тяжёлыми предметами"),
